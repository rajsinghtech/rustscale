//! tsrs-bench — throughput and latency benchmark harness for tailscale-rs.
//!
//! Modes:
...
#[command(
    name = "tsrs-bench",
    about = "Throughput and latency benchmark for tailscale-rs"
)]
struct Cli {
    #[command(subcommand)]
    command: Command,
...
enum Command {
    ...
    Server {
        ...
        hostname: String,
        /// Control-plane URL.
        #[arg(long, default_value = "https://controlplane.tailscale.com")]
        control_url: String,
        /// State directory (for key file storage).
        #[arg(long)]
...
async fn main() {
    ...
    let result = match cli.command {
        ...
        Command::Latency {
            ...
            let res =
                Box::pin(latency::run(authkey, target, count, hostname, control_url, state_dir))
                    .await;
            match res {
                Ok(r) => {
                    if json {
...
fn print_throughput_table(r: &throughput::ThroughputResult) {
    eprintln!();
    eprintln!("=== tsrs-bench throughput results ===");
...
fn print_latency_table(r: &latency::LatencyResult) {
    eprintln!();
    eprintln!("=== tsrs-bench latency results ===");
