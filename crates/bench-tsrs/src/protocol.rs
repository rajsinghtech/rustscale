//! Wire protocol between tsrs-bench client and server.
//!
...
/// Buffer size for firehose writes — kept at MTU (1280) to avoid overwhelming
/// the netstack's internal channel.
...
pub async fn write_header<T>(stream: &mut T, hdr: &Header) -> Result<(), Box<dyn Error>>
where
...
pub async fn read_header<T>(stream: &mut T) -> Result<Header, Box<dyn Error>>
where
    T: AsyncReadExt + AsyncWriteExt + Unpin,
{
    let mut buf = [0u8; HEADER_LEN];
...
pub async fn write_ack<T>(stream: &mut T) -> Result<(), Box<dyn Error>>
where
    T: AsyncReadExt + AsyncWriteExt + Unpin,
{
...
pub async fn read_ack<T>(stream: &mut T) -> Result<(), Box<dyn Error>>
where
    T: AsyncReadExt + AsyncWriteExt + Unpin,
{
