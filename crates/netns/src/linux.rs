...
fn use_socket_mark() -> bool {
    if std::env::var("TS_FORCE_LINUX_BIND_TO_DEVICE").is_ok() { return false; }
...
fn probe_socket_mark() -> bool {
    ...
    let addr = libc::sockaddr_in {
        sin_family: libc::AF_INET as u16, sin_port: 1u16.to_be(),
        sin_addr: libc::in_addr { s_addr: u32::from_be_bytes([127, 0, 0, 1]).to_be() },
        sin_zero: [0u8; 8],
    };
    let ret = unsafe {
        libc::connect(sock, (&raw const addr).cast::<libc::sockaddr>(),
            std::mem::size_of::<libc::sockaddr_in>() as libc::socklen_t)
    };
    if ret != 0 { unsafe { libc::close(sock) }; return true; }
    let mark: libc::c_int = LINUX_BYPASS_MARK as libc::c_int;
    let ret = unsafe {
        libc::setsockopt(sock, libc::SOL_SOCKET, libc::SO_MARK,
            (&raw const mark).cast::<libc::c_void>(),
            std::mem::size_of_val(&mark) as libc::socklen_t)
    };
...
fn ignore_errors() -> bool { unsafe { libc::getuid() != 0 } }
