...
pub async fn control_and_connect(addr: SocketAddr) -> Result<TcpStream, std::io::Error> {
    ...
    if !super::DISABLE_BIND_CONN_TO_INTERFACE.load(std::sync::atomic::Ordering::Relaxed) {
        if let Some(idx) = get_interface_index(addr) {
            let (level, opt) = if addr.is_ipv4() { (libc::IPPROTO_IP, IP_BOUND_IF) } else { (libc::IPPROTO_IPV6, IPV6_BOUND_IF) };
            let idx_val: libc::c_uint = idx;
            let ret = unsafe {
                libc::setsockopt(fd, level as libc::c_int, opt as libc::c_int,
                    (&raw const idx_val).cast::<libc::c_void>(),
                    std::mem::size_of_val(&idx_val) as libc::socklen_t)
            };
            if ret != 0 { eprintln!("netns: setsockopt IP_BOUND_IF failed for idx={idx}"); }
        }
    }
    ...
    if name.starts_with("utun") {
        if let Ok(ifaces) = if_addrs::get_if_addrs() {
            for iface in &ifaces {
                ...
                if is_ts { return None; }
            }
        }
    }
    let cname = match CString::new(name.as_str()) { Ok(c) => c, Err(_) => return None };
...
struct RtMsgHdr {
    rtm_msglen: u16, rtm_version: u8, rtm_type: u8, rtm_index: u16,
    rtm_flags: u32, rtm_addrs: i32, rtm_pid: i32, rtm_seq: i32,
    rtm_use: i32, rtm_inits: u32, rtm_rmx: [u8; 56],
}
...
#[allow(clippy::struct_field_names)]
struct SockaddrIn {
    sin_len: u8, sin_family: u8, sin_port: u16, sin_addr: [u8; 4], sin_zero: [u8; 8],
}

#[repr(C)]
#[allow(clippy::struct_field_names)]
struct SockaddrIn6 {
    sin6_len: u8, sin6_family: u8, sin6_port: u16, sin6_flowinfo: u32,
...
fn interface_index_for_route(ip: IpAddr) -> Option<u32> {
    ...
    let dest_bytes: Vec<u8> = match ip {
        IpAddr::V4(v4) => {
            let sa = SockaddrIn { sin_len: 16, sin_family: libc::AF_INET as u8, sin_port: 0, sin_addr: v4.octets(), sin_zero: [0u8; 8] };
            unsafe { std::slice::from_raw_parts((&raw const sa).cast::<u8>(), std::mem::size_of::<SockaddrIn>()).to_vec() }
        }
        IpAddr::V6(v6) => {
            let sa = SockaddrIn6 { sin6_len: 28, sin6_family: libc::AF_INET6 as u8, sin6_port: 0, sin6_flowinfo: 0, sin6_addr: v6.octets(), sin6_scope_id: 0 };
            unsafe { std::slice::from_raw_parts((&raw const sa).cast::<u8>(), std::mem::size_of::<SockaddrIn6>()).to_vec() }
        }
    };
    let msg_len = std::mem::size_of::<RtMsgHdr>() + dest_bytes.len();
    let mut msg = Vec::with_capacity(msg_len);
    let rtm = RtMsgHdr {
        rtm_msglen: msg_len as u16, rtm_version: RTM_VERSION, rtm_type: RTM_GET, rtm_index: 0,
        rtm_flags: RTF_UP, rtm_addrs: 1 << RTAX_DST, rtm_pid: unsafe { libc::getpid() },
        rtm_seq: 1, rtm_use: 0, rtm_inits: 0, rtm_rmx: [0u8; 56],
    };
    unsafe {
        let ptr = (&raw const rtm).cast::<u8>();
        msg.extend_from_slice(std::slice::from_raw_parts(ptr, std::mem::size_of::<RtMsgHdr>()));
    }
    msg.extend_from_slice(&dest_bytes);
    let written = unsafe { libc::write(fd, msg.as_ptr().cast(), msg.len()) };
    if written < 0 { unsafe { libc::close(fd) }; return None; }
...
fn parse_route_response(buf: &[u8]) -> Option<u32> {
    if buf.len() < std::mem::size_of::<RtMsgHdr>() { return None; }
...
#[allow(dead_code)]
fn _is_cgnat_v4(ip: Ipv4Addr) -> bool { super::is_cgnat_v4(ip) }
#[allow(dead_code)]
fn _is_tailscale_ula(ip: Ipv6Addr) -> bool { super::is_tailscale_ula(ip) }
