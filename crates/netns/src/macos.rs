        ...
        if let Some(idx) = get_interface_index(addr) {
            ...
            let ret = unsafe {
                libc::setsockopt(
                    ...
                    level as libc::c_int,
                    opt as libc::c_int,
                    &idx_val as *const _ as *const libc::c_void,
...
fn interface_index_for_route(ip: IpAddr) -> Option<u32> {
    ...
    unsafe {
        let ptr = &rtm as *const _ as *const u8;
...
fn parse_route_response(buf: &[u8]) -> Option<u32> {
    ...
    let rtm_addrs = {
        ...
        let addrs_offset = 16;
        let addrs_bytes = &hdr_bytes[addrs_offset..addrs_offset + 4];
        i32::from_le_bytes([addrs_bytes[0], addrs_bytes[1], addrs_bytes[2], addrs_bytes[3]])
    };
