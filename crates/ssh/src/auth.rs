...
pub fn eval_ssh_policy(policy: &SSHPolicy, info: &ConnInfo) -> EvalResult {
    ...
    for rule in &policy.Rules {
        match match_rule(rule, info) {
            Ok(MatchedRule { action, local_user, accept_env }) => {
                return EvalResult::Accept { action, local_user, accept_env };
            }
            Err(MatchError::UserMatch) => failed_on_user = true,
            Err(_) => {}
        }
    }
    if failed_on_user { EvalResult::RejectedUser } else { EvalResult::Rejected }
}

struct MatchedRule { action: SSHAction, local_user: String, accept_env: Vec<String> }
enum MatchError { NilAction, RuleExpired, PrincipalMatch, UserMatch }
...
fn principal_matches(p: &SSHPrincipal, info: &ConnInfo) -> bool {
    ...
    if !p.Node.is_empty() && p.Node == info.node.StableID { return true; }
    if !p.NodeIP.is_empty() {
        if let Ok(ip) = p.NodeIP.parse::<IpAddr>() {
            if ip == info.src_ip { return true; }
        }
    }
    if !p.UserLogin.is_empty() && info.user_profile.LoginName == p.UserLogin { return true; }
    false
}
...
mod tests {
    ...
    fn make_info(ssh_user: &str, src_ip: &str) -> ConnInfo {
        ConnInfo {
            ssh_user: ssh_user.into(), src_ip: src_ip.parse().unwrap(), dst_ip: "100.64.0.1".parse().unwrap(),
            node: Node { ID: 1, StableID: StableNodeID::from("nodeA"), Key: NodePrivate::generate().public(), ..Default::default() },
            user_profile: UserProfile { ID: 1, LoginName: "alice@example.com".into(), DisplayName: "Alice".into(), ProfilePicURL: String::new() },
        }
    }
    #[test]
    fn test_no_rules() { assert_eq!(eval_ssh_policy(&SSHPolicy { Rules: vec![] }, &make_info("root", "100.64.0.2")), EvalResult::Rejected); }
    #[test]
    fn test_accept_any() {
        let p = SSHPolicy { Rules: vec![SSHRule {
            Principals: vec![SSHPrincipal { Any: true, ..Default::default() }],
            SSHUsers: { let mut m = BTreeMap::new(); m.insert("*".into(), "=".into()); m },
            Action: Some(SSHAction { Accept: true, ..Default::default() }), ..Default::default()
        }]};
    ...
    fn test_reject_user() {
        let p = SSHPolicy { Rules: vec![SSHRule {
            Principals: vec![SSHPrincipal { Any: true, ..Default::default() }],
            SSHUsers: { let mut m = BTreeMap::new(); m.insert("root".into(), "root".into()); m },
            Action: Some(SSHAction { Accept: true, ..Default::default() }), ..Default::default()
        }]};
        assert_eq!(eval_ssh_policy(&p, &make_info("alice", "100.64.0.2")), EvalResult::RejectedUser);
    }
    #[test]
    fn test_principal_node_ip() {
        let p = SSHPolicy { Rules: vec![SSHRule {
            Principals: vec![SSHPrincipal { NodeIP: "100.64.0.2".into(), ..Default::default() }],
            SSHUsers: { let mut m = BTreeMap::new(); m.insert("*".into(), "root".into()); m },
            Action: Some(SSHAction { Accept: true, ..Default::default() }), ..Default::default()
        }]};
    ...
    fn test_principal_user_login() {
        let p = SSHPolicy { Rules: vec![SSHRule {
            Principals: vec![SSHPrincipal { UserLogin: "alice@example.com".into(), ..Default::default() }],
            SSHUsers: { let mut m = BTreeMap::new(); m.insert("*".into(), "=".into()); m },
            Action: Some(SSHAction { Accept: true, ..Default::default() }), ..Default::default()
        }]};
        match eval_ssh_policy(&p, &make_info("alice", "100.64.0.2")) {
