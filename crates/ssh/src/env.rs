...
mod tests {
    ...
    fn test_filter_env_empty() {
        assert!(filter_env(&[], &["TERM=xterm".into()]).unwrap().is_empty());
    }
    #[test]
    fn test_filter_env_exact() {
        assert_eq!(
            filter_env(&["TERM".into()], &["TERM=xterm".into(), "PATH=/bin".into()]).unwrap(),
            vec!["TERM=xterm"]
        );
    }
    #[test]
    fn test_filter_env_wildcard() {
        assert_eq!(
            filter_env(
                &["LC_*".into()],
                &["LC_CTYPE=en".into(), "LC_ALL=C".into(), "TERM=x".into()]
            )
            .unwrap(),
            vec!["LC_CTYPE=en", "LC_ALL=C"]
        );
    }
    #[test]
    fn test_filter_env_rejects_dangerous() {
        assert_eq!(
            filter_env(&["*".into()], &["LD_PRELOAD=/x".into(), "TERM=x".into()]).unwrap(),
            vec!["TERM=x"]
        );
    }
    #[test]
