...
pub struct Node {
    ...
    #[serde(default, skip_serializing_if = "skip_zero_machine", deserialize_with = "deserialize_null_to_default")]
    pub Machine: MachinePublic,
    /// The node's disco public key (zero if unset, then omitted).
    #[serde(default, skip_serializing_if = "skip_zero_disco", deserialize_with = "deserialize_null_to_default")]
...
pub struct NetInfo {
    ...
    /// `"regionID-v4"` / `"-v6"`.
    #[serde(default, skip_serializing_if = "BTreeMap::is_empty", deserialize_with = "deserialize_null_map_values")]
