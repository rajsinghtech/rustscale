...
| Syspolicy | 🔶 `crates/syspolicy` provides typed well-known definitions, device/profile/user scope precedence with per-item origins, immutable snapshots and callbacks, strict typed default/error conversion, concurrent deterministic provider merging, bounded Unix JSON/environment providers, cfg-safe unsupported-platform stubs, and race-safe scoped test overrides. `InstallUpdates` is enforced on the daemon's persisted `AutoUpdate` preference at startup. Deferred: native Windows registry/Apple managed-preference stores, file watching/debounced reloads, LocalAPI/CLI diagnostics, and broad live preference reconciliation. |
