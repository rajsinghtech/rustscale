...
| TCP connection table | `net/netstat/` | 🔶 `crates/netstat` provides bounded, cancellable, race-tolerant read-only TCP snapshots with normalized endpoints/states, strict Linux `/proc/net/tcp{,6}` parsing and bounded best-effort inode/PID association, plus a deadline/output-capped numeric macOS `netstat` fallback; readers are injectable and Windows returns explicit `Unsupported`. It is intentionally not wired into connection breaking: table rows do not safely identify an owned file descriptor, and enumeration never opens, closes, or duplicates socket FD targets. |
