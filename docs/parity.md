...
| Feature | Status |
| --- | --- |
| Tailscale SSH (`ssh/tailssh/`, port-10) | ✅ policy engine (Any/Node/NodeIP/UserLogin; only an unambiguous terminal `Accept` succeeds; Reject/malformed actions reject and unsupported HoldAndDelegate fails closed), policy-mapped local-account launch and recorder identity, incubator privilege drop + PTY, whois integration, and cancellation-safe orchestration. Shells run in dedicated process groups; recorder/client failure closes pumps and stdin, sends TERM then bounded KILL, while normal exit drains PTY/stdout and the recorder before channel close. Remote recording uses authenticated tailnet dials, HTTP/1.1 `/record` + h2c `/v2/record`, acknowledgement liveness, bounded buffering, and fail-open/reject/terminate semantics; deprecated local `.cast` recording requires `TS_DEBUG_LOG_SSH`. Remaining: delegated HoldAndDelegate fetch, recording-failure `NotifyURL` control callback, SFTP |
