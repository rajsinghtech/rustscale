...
| Outbound dial abstraction | `net/tsdial/` | ✅ `crates/tsdial`: `Dialer` with SystemDial/UserDial/PeerDial paths, `DnsMap` MagicDNS resolution, `UserDialPlan`, netmon link-change callback stub, `ActiveConns` tracking; UserDial follows ordinary OS/TUN routes without infrastructure's physical-underlay bypass and its generation map is refreshed from complete authorization-filtered map snapshots; route-aware callers can classify and dial one exact resolved address to avoid DNS/route TOCTOU; all direct product dial call sites use an explicit dial class |
...
route snapshot and holds that snapshot stable through exact-IP connect.
