...
| Taildrive (`drive/`) | 🔶 additive secure server layer: validated share model, capability-confined no-symlink paths, atomic disabled-by-default runtime config, signed self `drive:share` gating, packet-filter `tailscale.com/cap/drive` grants bound to transport-authenticated WireGuard key/address ownership, stable-`Node.ID` atomic key/address/tunnel/path/route reconciliation, map-epoch revocation, pre-body method/path/grant authorization, globally bounded PeerAPI admission, chunk-streamed atomic PUT, bounded/cancellable `/v0/drive` WebDAV level 1, and read-only LocalAPI status/config plus read-write replacement. Platform mounts, remote composition, persistence, UI/CLI, user switching, bookmarks, locks and range reads remain deferred. See `docs/taildrive.md`. |
