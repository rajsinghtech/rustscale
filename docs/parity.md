...
| IPN extension system | `ipn/ipnext/` | 🔶 `crates/ipnext`: deterministic registration, async startup rollback (including retained partial-init compensation failures), reverse dependency-ordered shutdown, and state/profile notifications are wired into tsnet. LocalAPI generation handoff leaves the advertised endpoint untouched until final atomic publication and fences stale accepted mutation handlers. Explicit close remains retryable; `Server::drop` instead uses a bounded global deadline, exponential backoff, and fixed attempt count after synchronously revoking network/publication authority. At that terminal bound it aborts joinable work and logs any non-cooperative extension callback or OS router cleanup as an intentional leak rather than keeping an unbounded thread. Control/netmap/filter/router and direct LocalAPI extension hooks remain deferred. |
...
| TCP connection table | `net/netstat/` | 🔶 `crates/netstat` provides hard-deadline/cancellable, worker-capped read-only TCP snapshots with normalized endpoints/states. Linux strictly parses `/proc/net/tcp{,6}` using native-endian 32-bit lanes, tolerates only `NotFound` family absence, and performs bounded best-effort inode/PID association only across matching pre/post process start times. macOS invokes wide numeric `netstat` and rejects truncated or ambiguous IPv6 instead of repairing it. Readers/decoders are injectable; Windows returns explicit `Unsupported`. PID metadata is diagnostic-only and the crate is intentionally not wired into connection breaking: table rows do not safely identify an owned file descriptor, and enumeration never opens, closes, or duplicates socket FD targets. |
...
| Taildrive (`drive/`) | 🔶 additive secure server layer: validated share model, capability-confined no-symlink paths, atomic disabled-by-default runtime config, signed self `drive:share` gating, packet-filter `tailscale.com/cap/drive` grants bound to transport-authenticated WireGuard key/address ownership, stable-`Node.ID` atomic key/address/tunnel/path/route reconciliation, linearizable map/config epoch revocation across every mutating WebDAV publication, pre-body method/path/grant authorization, globally bounded PeerAPI admission, chunk-streamed atomic PUT, bounded/cancellable `/v0/drive` WebDAV level 1, and read-only LocalAPI status/config plus read-write replacement. Platform mounts, remote composition, persistence, UI/CLI, user switching, bookmarks, locks and range reads remain deferred. See `docs/taildrive.md`. |
| Tailnet Lock / TKA (`tka/`) | 🔶 `crates/tka` provides canonical CBOR wire/hash types, direct/rotation/wrapped-credential verification, disablement checks, bounded authority/bootstrap/inform/fork resolution, signed update builders, and file-backed Chonk storage with crash-recovered batch rollback. `tsnet` performs authenticated, deadline/body/count-bounded same-session Noise bootstrap and two-way `/machine/tka/sync/*`, persists verified authority changes in owner-only profile/control namespaces, handles control disablement proofs, sends its TKA head, and intersects stable-ID-reconciled full/delta/patch netmaps with signature authorization fail-closed (including rotation obsolescence). A serialized TKA operation spans each control decision/apply and its shared peer commit/revocation barrier; head/control transitions and LocalAPI init/resume immediately withdraw PeerAPI/Taildrive provenance, tunnels, routes, magicsock and generation-stamped direct/DERP/relay ciphertext authorization before authority commit or synchronization can await the control plane. Authorized init/resume runs as a shared lifecycle-retained flight, so LocalAPI EOF, handler cancellation, close, or logout cannot abandon partial withdrawal/commit; cancellation retains the same JoinHandle and shutdown retries join it to observed completion before any router teardown, while failures stay `ready=false`. Local disable joins the same barrier, stale disabled maps cannot bypass revocation, and only a fresh map with a successfully validated ready authority can atomically republish peers; revoked ciphertext is rejected before enqueue or at the delivery barrier. `UnsignedPeerAPIOnly` nodes are conservatively dropped because restricted PeerAPI-only data-plane plumbing is deferred. Read-write-authorized LocalAPI and CLI flows cover status, self-safe init/resume, receipt acknowledgement, node signing, and disablement. Init durably receipts the original disablement secrets before control RPCs and reports ambiguous commit/drop outcomes without replacement generation; testcontrol exercises session binding, commit-then-drop recovery, filtering, partial-update rejection, signing, profile-isolated state recovery, and disable. Deferred: trusted-key add/remove CLI flows, affected-signature re-signing, local-disable denylisting, pre-auth-key wrapping, changelog UI, full compaction, and retroactive revocation orchestration. This is not full Tailnet Lock parity. |
| Device posture (`posture/`) | 🔶 production identity slice: exact PascalCase/null-tolerant C2N response, bounded Linux/macOS serial and non-loopback MAC collection, last-known MAC stability, and bounded live `PolicyEngine` snapshots for `always`/`never`/`user-decides` from Linux policy JSON, macOS `defaults`, and Windows machine-policy Registry providers. Provider failures and platforms without a provider fail closed. Posture prefs persist prospectively before serialized publication through one shared prefs state used by socket, loopback, and in-memory LocalAPI clients. Sensitive C2N has no production loopback listener (the legacy TCP harness is private and test-only): it runs only on the authenticated map Noise/H2 session, after map delivery, with strict HTTP limits/deadlines, four-request session concurrency, and disconnect cancellation. Hardware addresses are reported only for `hwaddrs=true`; serial collection remains unsupported beyond Linux/macOS. Arbitrary posture signals/attestation remain deferred. |
...
| Syspolicy | 🔶 `crates/syspolicy` provides typed well-known definitions, device/profile/user scope and explicit managed/debug precedence with per-item origins, immutable snapshots/callbacks, typed default/error conversion (including upstream fallback for malformed preference options while retaining item diagnostics), concurrent deterministic allowlisted provider merging, coalesced asynchronous notifications, bounded Unix JSON/environment providers, transactional removal/test overrides, plus bounded native macOS defaults and Windows HKLM providers for `PostureChecking`; native provider errors fail closed. `InstallUpdates` is enforced on persisted `AutoUpdate` and `Hostinfo.AllowsUpdate` (including managed denial over the remote-update env knob) at startup, every LocalAPI/config preference mutation, and live provider changes; startup uses a subscribe-first generation handshake. Deferred: broader native Windows and Apple managed-preference coverage, file watching, and LocalAPI/CLI policy diagnostics. |
...
Go-testcontrol-style test API); RequireAuth/CompleteAuth/
