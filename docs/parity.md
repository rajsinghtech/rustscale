...
| Device posture (`posture/`) | 🔶 production identity slice: exact PascalCase/null-tolerant C2N response, bounded Linux/macOS serial and non-loopback MAC collection, last-known MAC stability, live bounded Linux `always`/`never`/`user-decides` managed policy plus persisted preference updates, and same-map-session Noise C2N reporting with bounded HTTP parsing/deadlines. Hardware addresses are reported only when control requests `hwaddrs=true`; serial-number collection remains unsupported on other RustScale targets. Arbitrary posture signals/attestation and platform coverage beyond Linux/macOS remain deferred. |
