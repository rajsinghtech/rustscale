...
| Taildrive (`drive/`) | 🔶 additive `crates/drive` core: validated share/grant model, capability-confined no-symlink paths, atomic disabled-by-default config snapshots, bounded/cancellable WebDAV level-1 remote-share endpoint, and hermetic security/protocol tests. PeerAPI/LocalAPI/tsnet wiring is intentionally deferred until PeerAPI can supply exact authenticated `tailscale.com/cap/drive` grants; platform mounts, UI/CLI, user switching, bookmarks, locks, range reads and remote composition remain deferred. See `docs/taildrive.md`. |
