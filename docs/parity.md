...
| Key lifecycle | — | 🔶 | logout implemented: Server::logout sends control logout (Expiry=far past), clears persisted state + netmap cache, transitions to NeedsLogin; POST /logout wired via logout_trigger; key rotation/re-registration still ⬜ (TODO in map update task) |
