...
| OS-level route management | `wgengine/router/` | 🔶 `crates/router` provides incremental, teardown-safe command-backed `Router` implementations for macOS/Linux (including TUN route updates and exit-node toggles); `FakeRouter` unit-tests pure diffing. The crate also exposes the standalone Linux IP set manager described below. Phase 2: native PF_ROUTE/netlink plus remaining firewall integration. |
...
| Linux ipset | 🔶 `crates/router::ipset` provides a production `hash:ip` desired-state manager with strict set/name/family/address validation, stable diffs and argv-only `ipset` execution. Create/update/delete are idempotent, partial failures run journaled inverse commands, rollback failures preserve conservative ownership state, and teardown never adopts or destroys a pre-existing set. The executor is injectable and command vectors plus every forward failure point are unit-tested. Integration with a native nftables set or router/filter rule is deferred because no current RustScale firewall path has clear set-reference semantics. |
