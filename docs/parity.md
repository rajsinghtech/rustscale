...
| Taildrive (`drive/`) | ⬜ |
| Tailnet Lock / TKA (`tka/`) | 🔶 `crates/tka` core: CBOR wire types (AUM, NodeKeySignature, Key, State — CTAP2 canonical, integer keys, dup-key + 16-deep nesting rejection), BLAKE2s AUM/sig hashing (base32 AumHash), ed25519 signature-chain verification (verify_strict for Direct/Credential, plain for Rotation, depth cap 15, pubkey-match assertion), argon2id disablement check, Chonk trait + MemChonk; 45 tests. Part 2 (Authority sync, `/machine/tka/*` control RPCs, netmap peer filtering, FsChonk) not yet started |
