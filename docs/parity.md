...
| Feature | Status |
| --- | --- |
| Tailscale SSH (`ssh/tailssh/`, port-10) | ✅ policy engine (eval_ssh_policy with Any/Node/NodeIP/UserLogin, Reject/Accept), incubator (spawn shell with privilege drop + PTY slave fd dup2), whois integration, session orchestrator, and remote recording over authenticated tailnet dials with policy recorder selection, HTTP/1.1 `/record` + h2c `/v2/record`, acknowledgement liveness, bounded buffering, fail-open/reject/terminate semantics, and bounded teardown drain; deprecated local `.cast` recording is available only with `TS_DEBUG_LOG_SSH`. Remaining: HoldAndDelegate, recording-failure `NotifyURL` control callback, SFTP |
