#!/bin/sh
# rustscale source installer — builds from source and installs the C library +
# header (and optionally the rustscale-tun CLI).
#
# This is the "I have a Rust toolchain" path. For the zero-dependency binary
# installer, use install.sh instead:
#   curl -fsSL https://rajsinghtech.github.io/rustscale/install.sh | sh
#
# POSIX sh, set -eu, all logic wrapped in main() invoked at the bottom so a
