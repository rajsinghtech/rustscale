...
def runtime_metadata(run):
    ...
    inbound-pipeline flag, or predate the outbound sender flag. Callers that
    merely validate or aggregate them must preserve that absence.
    """
    if "runtime" not in run:
        return None
    runtime = run["runtime"]
    required = {"rs_tun_inbound_pipeline", "rs_tun_outbound_send_pipeline", "linux_udp_batch", "linux_udp_gro"}
    legacy = {"rs_tun_inbound_pipeline"}
    previous = {"rs_tun_inbound_pipeline", "linux_udp_batch", "linux_udp_gro"}
    if not isinstance(runtime, dict) or set(runtime) not in (required, previous, legacy) or any(type(value) is not bool for value in runtime.values()):
