  ...
  if [[ -f "$BENCH_LAST_FILE" ]]; then
    ...
    if [[ -n "$leftover" && "$leftover" != "null" ]]; then
      ...
      if [[ -n "$l_dns" && -n "$l_cid" && -n "$l_csec" ]]; then
        ...
        local lt
        lt=$(curl -fsS --connect-timeout 10 --max-time 20 \
          -X POST "$BENCH_API/api/v2/oauth/token" \
          -d client_id="$l_cid" -d client_secret="$l_csec" 2>/dev/null \
          | jq -r .access_token 2>/dev/null || echo "")
        if [[ -n "$lt" && "$lt" != "null" ]] &&
          curl -fsS --connect-timeout 10 --max-time 20 \
            --retry 3 --retry-delay 3 --retry-all-errors --retry-max-time 45 \
            -o /dev/null -X DELETE \
...
bench_mint_authkey() {
  ...
  local key
  # A full scale userspace cell can exceed 15 minutes. Keep the key valid for
  # the bounded matrix; the caller chooses whether control may reap disconnected
  # devices before the disposable tailnet is deleted.
...
bench_cleanup_tailnet() {
  if [[ -n "${BENCH_DNS:-}" ]]; then
    ...
    local t
    t=$(curl -fsS --connect-timeout 10 --max-time 20 \
      -X POST "$BENCH_API/api/v2/oauth/token" \
