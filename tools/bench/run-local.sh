#!/usr/bin/env bash
# tools/bench/run-local.sh — local A/B harness for rustscale-bench.
#
# Starts two rustscale-bench processes (server+client) joining an ephemeral
# tailnet, runs throughput + latency tests, and collects JSON results into
# bench-results/<timestamp>/rustscale.json.
#
# Usage:
#   source .secrets/tailscale.env && tools/bench/run-local.sh
#
# Options (env vars):
...
echo "[bench] rustscale local harness: $TS"

# Build the bench binary.
echo "[bench] building rustscale-bench..." >&2
cargo build -p rustscale-bench --release 2>&1 | tail -1 >&2
BENCH_BIN="target/release/rustscale-bench"
...
STATE_A=$(mktemp -d /tmp/rustscale-bench-a.XXXXXX)
STATE_B=$(mktemp -d /tmp/rustscale-bench-b.XXXXXX)
...
echo "$THROUGHPUT_JSON" | jq '.' > "$RESULTS_DIR/rustscale-throughput.json"

# ---------------------------------------------------------------------------
# Run latency test (reuse the same client state dir for key persistence).
# ---------------------------------------------------------------------------
STATE_C=$(mktemp -d /tmp/rustscale-bench-lat.XXXXXX)
...
echo "$LATENCY_JSON" | jq '.' > "$RESULTS_DIR/rustscale-latency.json"
...
jq -n \
  ...
  '{tool: "rustscale-bench", stamp: $stamp, throughput: $throughput, latency: $latency}' \
  > "$RESULTS_DIR/rustscale.json"

echo "[bench] results saved to $RESULTS_DIR/rustscale.json" >&2
echo ""
echo "═══ rustscale-bench results ═══"
...
echo "  results:  $RESULTS_DIR/rustscale.json"
