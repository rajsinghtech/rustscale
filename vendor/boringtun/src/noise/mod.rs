...
static NEXT_PIPELINE_TUNNEL_ID: AtomicU64 = AtomicU64::new(1);
...
pub struct Tunn {
    ...
    /// Changes only when a speculative established-data result can no longer
    /// be committed (session/handshake or queued-output eligibility changes).
    pipeline_generation: u64,
    /// Stable per-instance identity for opaque pipeline capabilities.
...
/// Failed speculative open with its provided reusable plaintext slot intact.
#[derive(Debug)]
pub struct OpenError {
    pub error: WireGuardError,
    pub plaintext: Vec<u8>,
}

#[derive(Debug)]
pub struct CommitError {
    /// The stale capability still owns its caller-provided reusable slot.
    opened: OpenedData,
}

impl CommitError {
    fn stale(opened: OpenedData) -> Self {
        Self { opened }
    }

    /// Recover the slot after a mutation-free stale commit rejection.
    pub fn into_plaintext(self) -> Vec<u8> {
        self.opened.into_plaintext()
    }
}
...
impl Tunn {
    ...
    pub fn new(
        ...
        Tunn {
            ...
            pipeline_tunnel_id: NEXT_PIPELINE_TUNNEL_ID.fetch_add(1, Ordering::Relaxed),
        }
    }
    ...
    /// Invalidate outstanding receive preparation when an operation changes
    /// session or queued output eligibility. Ordinary established-data I/O
    /// deliberately does not call this: its token remains valid until ordered
    /// replay validation decides otherwise.
    pub fn invalidate_pipeline_generation(&mut self) {
        self.pipeline_generation = self.pipeline_generation.wrapping_add(1);
    }

    /// Complete mutation-free established-data preflight. The caller holds the
    /// peer guard synchronously; no borrow is retained in the result.
    pub fn preflight_data(&self, datagram: &[u8]) -> Result<PreparedData, WireGuardError> {
        if !self.packet_queue.is_empty() {
            return Err(WireGuardError::NoCurrentSession);
        }
        let packet = match Self::parse_incoming_packet(datagram)? {
            Packet::PacketData(packet) => packet,
            _ => return Err(WireGuardError::InvalidPacket),
        };
        let session = self.sessions[(packet.receiver_idx as usize) % N_SESSIONS]
            .as_ref()
            .ok_or(WireGuardError::NoCurrentSession)?;
        if session.receiving_index != packet.receiver_idx {
            return Err(WireGuardError::WrongIndex);
        }
        session.receiving_counter_quick_check(packet.counter)?;
        Ok(PreparedData {
            receiver_idx: packet.receiver_idx,
    ...
    /// Copy and AEAD-open into an owned reusable slot. Every failure returns
    /// that slot to the caller in [`OpenError`].
    pub fn open_prepared_data(
        &self,
        datagram: &[u8],
        prepared: &PreparedData,
        plaintext: Vec<u8>,
    ...
    pub fn open_prepared_data_detached(
        ...
        let packet = match Self::parse_incoming_packet(datagram) {
            ...
            Packet::PacketData(packet)
                if packet.receiver_idx == prepared.receiver_idx
                    && packet.counter == prepared.counter =>
            {
                packet
            }
        ...
        let encrypted_body = &datagram[DATA_OVERHEAD_SZ - AEAD_TAG_SIZE..];
        let encrypted_body_len = encrypted_body.len();
        let aead_tag = match encrypted_body[encrypted_body.len() - AEAD_TAG_SIZE..].try_into() {
            Ok(tag) => tag,
    ...
    /// Validate every immutable part of an opened capability before the first
    /// ordered replay mutation in a burst.
    pub fn validate_opened_data(&self, opened: &OpenedData) -> bool {
        let prepared = &opened.prepared;
        self.pipeline_generation == prepared.generation
            && self.pipeline_tunnel_id == prepared.tunnel_id
            && self.packet_queue.is_empty()
            && opened.plaintext_len <= opened.plaintext.len()
            && opened.encrypted_body_len >= AEAD_TAG_SIZE
            && opened.aead_tag.len() == AEAD_TAG_SIZE
            && self.sessions[(prepared.receiver_idx as usize) % N_SESSIONS]
                .as_ref()
                .is_some_and(|session| {
                    session.receiving_index == prepared.receiver_idx
                        && session.receiving_counter_quick_check(prepared.counter).is_ok()
                })
    }

    /// Commit one already-authenticated item. Callers must first validate all
    /// tokens for their complete burst; stale is mutation-free.
    pub fn commit_opened_data(
        &mut self,
        opened: OpenedData,
    ) -> Result<(Vec<u8>, Option<usize>), CommitError> {
        let prepared = &opened.prepared;
        if self.pipeline_generation != prepared.generation
            || self.pipeline_tunnel_id != prepared.tunnel_id
            || !self.packet_queue.is_empty()
        {
            return Err(CommitError::stale(opened));
        }
        let Some(session) = self.sessions[(prepared.receiver_idx as usize) % N_SESSIONS].as_ref() else {
            return Err(CommitError::stale(opened));
        };
        if session.receiving_index != prepared.receiver_idx {
            return Err(CommitError::stale(opened));
        }
        // Complete-burst validation happened before the first mutation. A
        // counter can still become invalid because an earlier item in this
        // same FIFO burst advanced the replay window; scalar handling simply
        // drops it, so it must not become a stale-burst fallback or panic.
        if session.receiving_counter_quick_check(prepared.counter).is_err()
            || session.mark_received(prepared.counter).is_err()
        {
            return Ok((opened.plaintext, None));
        }
        self.set_current_session(prepared.receiver_idx as usize);
        self.timer_tick(TimerName::TimeLastPacketReceived);
        let mut plaintext = opened.plaintext;
        let accepted_len = match self.validate_decapsulated_packet(&mut plaintext[..opened.plaintext_len]) {
            TunnResult::Done => None,
            TunnResult::WriteToTunnelV4(packet, _) | TunnResult::WriteToTunnelV6(packet, _) => {
                Some(packet.len())
            }
            // Scalar `handle_data` has already committed replay/current/
            // packet-received state before rejecting malformed plaintext.
            // Preserve that state and publish no packet.
            TunnResult::Err(_) => None,
            // Data validation has no reply path; keep this defensive branch
            // scalar-equivalent (no plaintext publication) and non-panicking.
            TunnResult::WriteToNetwork(_) => None,
        };
        Ok((plaintext, accepted_len))
    }
    ...
    pub fn format_handshake_initiation<'a>(
        ...
        match self.handshake.format_handshake_initiation(dst) {
            Ok(packet) => {
                // Formatting changed handshake/output eligibility.
                self.invalidate_pipeline_generation();
...
impl OpenedData {
    /// Recover the owned slot when a whole speculative burst is abandoned.
    pub fn into_plaintext(self) -> Vec<u8> {
        self.plaintext
    }
}
...
mod tests {
    ...
    #[test]
    fn prepared_open_defers_replay_marking_until_commit() {
        let (mut sender, mut receiver) = create_two_tuns_and_handshake();
        let packet = create_ipv4_udp_packet();
        let mut send_dst = [0u8; 2048];
        let ciphertext = match sender.encapsulate(&packet, &mut send_dst) {
            TunnResult::WriteToNetwork(data) => data.to_vec(),
            _ => unreachable!(),
        };

        let prepared = receiver.preflight_data(&ciphertext).unwrap();
        let plaintext = vec![0u8; 2048];
        let opened = receiver
            .open_prepared_data(&ciphertext, &prepared, plaintext)
            .unwrap();
        // Opening alone must not consume the replay counter.
        assert!(receiver.preflight_data(&ciphertext).is_ok());
        assert!(matches!(
            receiver.commit_opened_data(opened).unwrap(),
            (_, Some(len)) if len == packet.len()
        ));
        assert!(receiver.preflight_data(&ciphertext).is_err());
    }

    #[test]
    fn bad_tag_and_stale_token_do_not_commit() {
        let (mut sender, mut receiver) = create_two_tuns_and_handshake();
        let packet = create_ipv4_udp_packet();
        let mut send_dst = [0u8; 2048];
        let ciphertext = match sender.encapsulate(&packet, &mut send_dst) {
        ...
        *corrupt.last_mut().unwrap() ^= 1;
        let plaintext = vec![0u8; 2048];
        assert!(receiver
            .open_prepared_data(&corrupt, &prepared, plaintext)
            .is_err());
        // The valid packet with the identical counter remains eligible.
        let prepared = receiver.preflight_data(&ciphertext).unwrap();
        let opened = receiver
            .open_prepared_data(&ciphertext, &prepared, vec![0u8; 2048])
            .unwrap();
        receiver.invalidate_pipeline_generation();
        assert!(matches!(
            receiver.commit_opened_data(opened),
            Err(_)
        ));
        assert!(receiver.preflight_data(&ciphertext).is_ok());
    }
