...
pub struct Interface {
    ...
    socket_egress_cursor: usize,
    socket_egress_burst: usize,
    socket_egress_remaining: usize,
}
...
impl Interface {
    ...
    pub fn new(config: Config, device: &mut (impl Device + ?Sized), now: Instant) -> Self {
        ...
        Interface {
            ...
            socket_egress_cursor: 0,
            socket_egress_burst: 1,
            socket_egress_remaining: 1,
    ...
    /// Set the maximum number of consecutive packets emitted from one socket.
    ///
    /// The default of one preserves strict round-robin egress. A larger bound
    /// preserves short same-flow runs for downstream packet aggregation while
    /// the socket cursor still prevents an active flow from monopolizing the
    /// device queue.
    pub fn set_socket_egress_burst(&mut self, packets: usize) {
        assert!(packets > 0, "socket egress burst must be non-zero");
        self.socket_egress_burst = packets;
        self.socket_egress_remaining = packets;
    }
    ...
    fn socket_egress(
        ...
        let socket_storage_len = sockets.storage_len();
        for (socket_index, item) in sockets.items_mut_from(self.socket_egress_cursor) {
            if !item
                .meta
            ...
            match result {
                Err(EgressError::Exhausted) => {
                    // Resume at the socket that could not claim a device slot.
                    // This makes partial refills fair without changing stable
                    // SocketHandle indices or requiring a full queue drain.
                    if socket_index != self.socket_egress_cursor {
                        self.socket_egress_remaining = self.socket_egress_burst;
                    }
                    self.socket_egress_cursor = socket_index;
                    break;
                }
                ...
                Ok(()) => {
                    if emitted {
                        ...
                        self.socket_egress_remaining -= 1;
                        if self.socket_egress_remaining == 0 {
                            self.socket_egress_cursor = (socket_index + 1) % socket_storage_len;
                            self.socket_egress_remaining = self.socket_egress_burst;
                        } else {
                            // Re-enter socket egress at this same socket so a
                            // bounded run remains contiguous in the device
                            // queue. `Interface::poll` immediately calls us
                            // again after a state-changing emission.
                            break;
                        }
                    }
                }
