...
impl InterfaceInner {
    pub(crate) fn process_tcp<'frame>(
        ...
        #[cfg(feature = "alloc")]
        {
            ...
            if let Some(handle) = sockets.tcp_flow_handle(remote, local) {
                if let Some(tcp_socket) = sockets.get_mut_checked::<Socket>(handle)
                ...
                // A stale entry must never suppress the authoritative scan.
                sockets.forget_tcp_flow(remote, local);
            }
        }
