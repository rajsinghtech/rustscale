...
#[test]
fn socket_egress_resumes_at_exhausted_socket() {
    ...
    struct LimitedTxToken<'a>(&'a mut Vec<Vec<u8>>);

    impl crate::phy::TxToken for LimitedTxToken<'_> {
    ...
    impl Device for LimitedDevice {
        type RxToken<'a> = crate::tests::RxToken;
        type TxToken<'a> = LimitedTxToken<'a>;
        ...
        fn transmit(&mut self, _timestamp: Instant) -> Option<Self::TxToken<'_>> {
            ...
            self.remaining -= 1;
            Some(LimitedTxToken(&mut self.packets))
        }
    ...
    let remote = IpEndpoint::new(IpAddress::v4(192, 168, 1, 2), 9000);
    for local_port in [1000, 2000] {
        let rx = udp::PacketBuffer::new(vec![udp::PacketMetadata::EMPTY], vec![0; 16]);
        let tx = udp::PacketBuffer::new(vec![udp::PacketMetadata::EMPTY; 2], vec![0; 32]);
        let mut socket = udp::Socket::new(rx, tx);
        socket.bind(local_port).unwrap();
        socket.send_slice(&[1], remote).unwrap();
        socket.send_slice(&[2], remote).unwrap();
        sockets.add(socket);
    }

    let mut device = LimitedDevice {
        remaining: 1,
        packets: Vec::new(),
    };
    assert_eq!(
    ...
    let source_ports: Vec<u16> = device
        .packets
        .iter()
        .map(|packet| {
            let ip = Ipv4Packet::new_checked(packet.as_slice()).unwrap();
            UdpPacket::new_checked(ip.payload()).unwrap().src_port()
        })
        .collect();
    assert_eq!(source_ports, vec![1000, 2000]);

    let (mut iface, mut sockets, _) = setup(Medium::Ip);
    iface.set_socket_egress_burst(2);
    for local_port in [1000, 2000] {
        let rx = udp::PacketBuffer::new(vec![udp::PacketMetadata::EMPTY], vec![0; 16]);
        let tx = udp::PacketBuffer::new(vec![udp::PacketMetadata::EMPTY; 2], vec![0; 32]);
        let mut socket = udp::Socket::new(rx, tx);
    ...
    let source_ports: Vec<u16> = device
        ...
        .collect();
    assert_eq!(source_ports, vec![1000, 1000, 2000, 2000]);
}
