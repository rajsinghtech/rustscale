...
impl<'a> Socket<'a> {
    ...
    /// Return the peer's effective advertised receive window after applying
    /// the negotiated RFC 1323 scaling factor.
    pub fn remote_window(&self) -> usize {
        self.remote_win_len
    }

    /// Return the current congestion-control window in bytes.
    ...
    /// Return whether a retransmission timer is active for unacknowledged data.
    pub fn retransmission_timer_active(&self) -> bool {
        self.timer.is_retransmit()
    }

    /// Return whether the socket is waiting to probe a zero-sized peer window.
    pub fn zero_window_probe_active(&self) -> bool {
        self.timer.is_zero_window_probe()
    }

    /// Return the current consecutive duplicate-ACK count.
    pub fn duplicate_ack_count(&self) -> u8 {
    ...
    pub(crate) fn process(
            ...
            && self.ack_to_transmit()
        {
            self.ack_delay_timer = match self.ack_delay_timer {
                AckDelayTimer::Idle if self.immediate_ack_to_transmit() => {
                    // Receive offload can deliver more than one RMSS of data
                    // in a single packet. Do not wait for a second packet to
                    // notice that the immediate-ACK threshold was crossed.
                    tcp_trace!("forcing immediate ack for aggregated data");
...
mod test {
    ...
    #[test]
    fn test_delayed_ack_aggregated_packet_larger_than_rmss() {
        let mut s = socket_established_with_buffer_sizes(DEFAULT_MSS * 2, DEFAULT_MSS * 2);
        s.set_ack_delay(Some(ACK_DELAY_DEFAULT));
        send!(
            s,
        ...
        // GRO can deliver several wire segments as one packet. The aggregate
        // already crosses the immediate-ACK threshold, so waiting for another
        // packet would incorrectly halve the receiver's ACK cadence.
        recv!(
            s,
            Ok(TcpRepr {
                seq_number: LOCAL_SEQ + 1,
